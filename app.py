...
if df is None:
    st.error("❌ 파일을 찾을 수 없습니다. '비교프로젝트_설비.xlsx' 파일이 같은 폴더에 있는지 확인해주세요.")
else:
    ...
    if st.button("🚀 프로젝트 조회", use_container_width=True):
        ...
        for j in project_cols:
            ...
            # [규칙 2] 위치: 프로젝트명에 해당 지역명이 포함되는지
            m_loc = (v_loc == "전체") or (v_loc in p_name)
            
            # [규칙 3] 년도: 3행(인덱스 2)
            m_year = (v_year == "전체") or (v_year in str(df.iloc[2, j]))
            
            # [규칙 4] 냉난방방식: 4행(인덱스 3) -> "개별가스" 등
            m_hvac = (v_hvac == "전체") or (v_hvac in str(df.iloc[3, j]))
            
            # [규칙 5] 건물유형: 5행(인덱스 4)
            m_type = (v_type == "전체") or (v_type in str(df.iloc[4, j]))

            # [규칙 6] 세대수: 6행(아파트)+7행(오피) (인덱스 5, 6) 합산
            ...
            # [규칙 8] 소방포함: 5행 비고(인덱스 4, j+1)
            fire_txt = str(df.iloc[4, j+1]) if pd.notna(df.iloc[4, j+1]) else ""
            m_fire = (v_fire == "전체") or (v_fire in fire_txt)

            # [규칙 9] 특화설비(인덱스 45), 특수사항(인덱스 47)
        ...
        if found_indices:
            ...
            st.warning("🧐 일치하는 현장이 없습니다. 조건을 조금 더 넓게 설정해보세요.")
